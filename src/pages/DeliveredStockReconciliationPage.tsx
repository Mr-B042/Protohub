...
export default function DeliveredStockReconciliationPage() {
  ...
  const openRows = useMemo(() => rows.filter(outstanding), [rows]);
  ...
  const productHistoryRows = useMemo(() => {
    ...
    return rows.filter((row) => row.agentId === agentKey && row.productId === productKey)
      .sort((a, b) => String(a.deliveredAt).localeCompare(String(b.deliveredAt))).slice(-200);
  }, [rows, agentKey, productKey]);
  ...
  return <div className="space-y-3 text-gray-900">
    ...
    {loading && rows.length === 0 ? <div className="rounded-xl border border-gray-200 bg-white p-16 text-center text-sm text-gray-400">Loading delivered stock…</div> : openRows.length === 0 ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-16 text-center"><CheckCircle2 className="mx-auto h-10 w-10 text-emerald-600" /><h2 className="mt-3 font-black text-emerald-900">All delivered stock is reconciled</h2><p className="mt-1 text-sm text-emerald-700">New delivered orders will appear here automatically.</p></div> : <div className="grid gap-3 xl:grid-cols-[0.8fr_0.8fr_1.6fr]">
