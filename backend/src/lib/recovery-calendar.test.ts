...
const TARGETS: RecoveryDayTargets = { followUp: 10, retention: 10, delivered: 1 };
const TODAY = "2026-08-24";
const counts = (over: Partial<RecoveryDayCounts> = {}): RecoveryDayCounts =>
  ({ day: "2026-08-20", followUp: 10, retention: 10, delivered: 1, ...over });
...
test("Sunday is a rest day, never a failure", () => {
  ...
  assert.equal(dayStatus(counts({ day: "2026-08-23", followUp: 0, retention: 0, delivered: 0 }), TARGETS, TODAY), "rest");
});
...
test("a past day with zero activity is critical, not blank", () => {
  assert.equal(dayStatus(counts({ day: "2026-08-20", followUp: 0, retention: 0, delivered: 0 }), TARGETS, TODAY), "critical");
});
...
test("a target of zero is skipped, not treated as instantly met", () => {
  const noDelivered: RecoveryDayTargets = { followUp: 10, retention: 10, delivered: 0 };
...
test("with no targets at all a day cannot be judged", () => {
  const none: RecoveryDayTargets = { followUp: 0, retention: 0, delivered: 0 };
...
test("critical days are counted inside below-target days", () => {
  const byDay = new Map([
    ["2026-08-20", { day: "2026-08-20", followUp: 0, retention: 0, delivered: 0 }],
    ["2026-08-21", { day: "2026-08-21", followUp: 7, retention: 10, delivered: 1 }],
    ["2026-08-22", { day: "2026-08-22", followUp: 10, retention: 10, delivered: 1 }]
  ]);
  const summary = buildRecoveryCalendar(
...
test("totals add up across the range", () => {
  const byDay = new Map([
    ["2026-08-20", { day: "2026-08-20", followUp: 4, retention: 7, delivered: 1 }],
    ["2026-08-21", { day: "2026-08-21", followUp: 6, retention: 5, delivered: 2 }]
  ]);
...
test("negative or junk counts never leak through as negatives", () => {
  const byDay = new Map([["2026-08-20", { day: "2026-08-20", followUp: -5, retention: Number.NaN, delivered: 1 }]]);
