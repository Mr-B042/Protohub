...
test("inactive managers with overdue queues escalate accountability risk", () => {
  const result = buildManagerPerformance(
    [{ id: "team-1", name: "North", leadId: "lead-1", productIds: [], memberIds: ["rep-1"] }],
    [
      { id: "lead-1", name: "Lead", active: true, lastSeenAt: daysAgo(1) },
      { id: "rep-1", name: "Rep", active: true }
    ],
    [
      {
        id: "o1",
  ...
  assert.equal(result.rows[0]?.managerOversightStatus, "escalated");
  assert.equal(result.rows[0]?.managerActionsToday, 0);
  assert.equal(result.rows[0]?.managerAccountability, 0);
  assert.ok((result.rows[0]?.score ?? 0) < 40);
});

test("manager reviews improve accountability without changing delivery weighting", () => {
  const baseArgs = [
    [{ id: "team-1", name: "North", leadId: "lead-1", productIds: [], memberIds: ["rep-1"] }],
    [
      { id: "lead-1", name: "Lead", active: true, lastSeenAt: minutesAgo(30) },
      { id: "rep-1", name: "Rep", active: true }
    ],
    [
      {
  ...
  const withoutReview = buildManagerPerformance(...baseArgs);
  const withReview = buildManagerPerformance(
    ...baseArgs,
    [],
    [],
    [
      {
        ...
        actorName: "Lead",
        orderId: "o1",
        actionType: "reviewed_queue",
        createdAt: minutesAgo(5)
      }
    ]
  );

  assert.ok((withReview.rows[0]?.managerAccountability ?? 0) > (withoutReview.rows[0]?.managerAccountability ?? 0));
  assert.equal(withReview.rows[0]?.reviewedActionableQueue, 1);
  assert.equal(withReview.rows[0]?.unreviewedActionableQueue, 0);
});
