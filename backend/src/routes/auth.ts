...
router.get("/team", requireAuth, async (req, res) => {
  let query = supabase
    .from("users")
    .select("id, name, email, phone, role, active, created_at, round_robin_position, round_robin_excluded, is_demo, is_head_of_sales_rep, head_of_sales_rep_appointed_at, last_seen_at, permissions, extra_pages, agent_balance_scope_mode, agent_balance_state_scope, agent_balance_agent_ids, marketing_attribution_tags")
...
router.patch("/team/:id", requireAuth, async (req, res) => {
  ...
  const VALID_ROLES = ["Owner", "Admin", "Manager", "Sales Rep", "Inventory Manager", "Inventory Manager & Logistics Operations", "Marketer", "Viewer", "Recovery Rep", "Delivery Agent", "Sales Closer"] as const;
...
router.post("/invite", requireAuth, async (req, res) => {
  ...
  const Schema = z.object({
    ...
    password: z.string().min(8).max(200),
    role: z.enum(["Admin", "Manager", "Sales Rep", "Inventory Manager", "Inventory Manager & Logistics Operations", "Marketer", "Viewer", "Recovery Rep", "Delivery Agent", "Sales Closer"]),
