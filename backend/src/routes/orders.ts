...
router.patch("/:id/status", requireRole("Owner", "Admin", "Manager", "Sales Rep", "Recovery Rep"), async (req, res) => {
  ...
  try {
    if (await confirmationNeedsSalesExpansionLog(req.user!.orgId, existing, callOutcome)) {
      res.status(409).json({
        error: `Complete the Upsell & Cross-sell Log before marking this order ${callOutcome}.`,
    ...
    logger.error("sales expansion status gate failed", { orderId: existing.id, status, callOutcome, error: error?.message });
...
router.patch("/:id", requireRole("Owner", "Admin", "Manager", "Sales Rep", "Recovery Rep", "Sales Closer"), async (req, res) => {
  ...
  const inventoryShapeChanged = INVENTORY_SHAPING_FIELDS.some((field) =>
